...
export default function Talleres() {
  ...
  return (
    <div className="space-y-6">
      ...
      <ComponentCard
        title={verHistorial ? "Historial de Talleres" : "Talleres Activos"}
        desc={verHistorial ? "Registro de talleres finalizados o inactivos" : "Talleres planificados actualmente activos"}
      >
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-3 mb-4">
          ...
          <div className="flex gap-2">
            <button onClick={() => { setVerHistorial(!verHistorial); setCurrentPage(1); }}
              className={`font-semibold py-2.5 px-4 rounded-lg shadow-sm transition-colors flex items-center justify-center gap-2 text-sm whitespace-nowrap border ${
                verHistorial
              ...
              {verHistorial ? "Ver Activos" : "Historial"}
            </button>
            {!verHistorial && (
              ...
              <tr key={t.id_taller} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                ...
                <td className="px-3 py-2 text-center whitespace-nowrap">
                  <div className="flex items-center justify-end gap-0.5">
                    ...
                    <button onClick={async () => { try { await mavetApi.getDocumentoPlan(t.id_taller); toast.success('PDF de planificación descargado'); } catch { toast.error('Error al descargar planificación'); } }}
                      className="flex flex-col items-center p-1 text-amber-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-500/10 rounded-lg transition-colors group" title="Plan Programático">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                      <span className="text-[9px] leading-tight opacity-70 group-hover:opacity-100">Plan</span>
                    </button>
                    {verHistorial ? (
                      ...
                      <>
                        ...
                        <button onClick={() => handleEliminarPlanificado(t)}
                          className="flex flex-col items-center p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 rounded transition-colors group" title="Eliminar">
      ...
      <ComponentCard
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setVerHistorialInsc(!verHistorialInsc)}
              className={`font-semibold py-2 px-3 rounded-lg shadow-sm transition-colors flex items-center justify-center gap-1.5 text-xs whitespace-nowrap border ${
                verHistorialInsc
                  ? "bg-brand-500 text-white border-brand-500 hover:bg-brand-600"
                  : "bg-white text-gray-700 border-gray-300 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700"
              }`}>
              ...
              {verHistorialInsc ? "Ver Activas" : "Historial"}
            </button>
          </div>
        }
      >
          ...
          <div className="flex flex-col items-center gap-2 py-10 text-center">
            ...
            <p className="text-sm font-medium text-gray-500">
              {verHistorialInsc
                ...
                : "No hay alumnos inscritos activos en ning&uacute;n taller."}
            </p>
          </div>
        ) : (
              ...
              <details key={idx} className="group rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                <summary className="flex items-center justify-between px-5 py-3.5 bg-gray-50 dark:bg-gray-800/50 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors list-none">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={async (e) => { e.stopPropagation(); try { await mavetApi.exportInscripciones(grupo.taller?.id_taller, 'pdf'); toast.success('PDF de inscritos descargado'); } catch { toast.error('Error al exportar'); } }}
                      className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors" title="Exportar inscritos PDF">
                      ...
                      {grupo.alumnos.map((ins: any, i: number) => (
                        <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                          ...
                          <td className="px-3 py-1.5 whitespace-nowrap">
                            {ins.estado_inscripcion === "Inscrito" || ins.estado_inscripcion === "Activo" ? (
                              ...
                              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[11px] font-semibold bg-gray-100 text-gray-500 dark:bg-gray-500/10 dark:text-gray-400">
                                <span className="w-1 h-1 rounded-full bg-gray-400" />
                                {ins.estado_inscripcion || "Inactivo"}
                              </span>
                            )}
                          </td>
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button type="button" onClick={handleSaveInstructor} disabled={isSubmitting || !personaEncontrada}
                        ...
                        <td className="px-3 py-2 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <button
                              ...
                              className="p-1 text-gray-400 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-500/10 rounded transition-colors"
                              title="Editar instructor"
                            >
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                            </button>
                            <button
                              onClick={() => handleDeleteInstructor(inst.id_instructor, `${inst.Persona?.nombres || ""} ${inst.Persona?.apellidos || ""}`)}
                              className="p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-500/10 rounded transition-colors"
